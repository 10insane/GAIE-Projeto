                    ...
                    [
                                ...
                                ft.Container(
                                    content=ft.Row(
                                        [
                                            ...
                                            ft.Text(nome_aluno, size=10, color=cor_texto_medio, weight=ft.FontWeight.W_500, max_lines=1, overflow=ft.TextOverflow.ELLIPSIS)
                                        ],
                                        spacing=4
                                    ),
                                ...
                                ft.Container(
                                    content=ft.Row(
                                        [
                                            ...
                                            ft.Text(registo.get("Estado", "Sem estado"), size=10, color=cor_texto_medio, weight=ft.FontWeight.W_500, max_lines=1, overflow=ft.TextOverflow.ELLIPSIS)
                                        ],
                                        spacing=4
                                    ),
                        ...
                        ft.Row(
                            [
                                ...
                                ft.Text(registo.get("DataEntradaSPO", "Data indisponível"), size=10, color=cor_texto_medio),
                                ft.Text("•", size=10, color=cor_texto_medio),
                                ft.Icon(ft.Icons.PERSON_OUTLINE, size=10, color=cor_texto_medio),
                                ft.Text(registo.get("NomeTecnico", "Sem técnico"), size=10, color=cor_texto_medio, max_lines=1, overflow=ft.TextOverflow.ELLIPSIS)
                            ],
                            spacing=4,
                            alignment=ft.MainAxisAlignment.START
                        ),
...
def criar_registos_view(registos, page):
    """View principal da lista de registos - Admin com filtros iguais ao normal"""
    ...
    if not registos:
        ...
        return ft.Container(
            content=ft.Column(
                [
                    ft.Container(
                        content=ft.Icon(ft.Icons.ASSIGNMENT_OUTLINED, size=80, color=cor_texto_medio),
    ...
    # Estado da view
    estado = {"pagina":0, "total":len(registos), "filtro_aluno_ou_numero":"", "filtro_estado":""}
    ...
    def carregar_pagina():
        ...
        filtro_aluno_ou_numero = estado["filtro_aluno_ou_numero"].strip().lower()
        ...
        for r in registos:
            ...
            if filtro_aluno_ou_numero:
                ...
                if filtro_aluno_ou_numero not in nome and filtro_aluno_ou_numero not in numero_aluno:
                    passa_filtro = False
    ...
    # — Header + filtros —
    info_resultados = ft.Text(f"{min(PAGE_SIZE,len(registos))} de {len(registos)} registos", size=12, color=cor_texto_medio, weight=ft.FontWeight.W_500)
            ...
            [
                ...
                ft.Row(
                    [
                        ...
                        ft.Column(
                            [
                                ft.Text("Lista de Registos", size=20, weight=ft.FontWeight.W_700, color=cor_texto_claro),
                                ft.Text("Gestão de registos de alunos", size=12, color=cor_texto_medio)
                            ],
                            spacing=1
                        ),
                        ...
                        ft.Container(
                            content=ft.Column(
                                [
                                    ft.Text(str(len(registos)), size=20, weight=ft.FontWeight.W_700, color=cor_secundaria),
                                    ft.Text("Total", size=10, color=cor_texto_medio)
                                ],
                                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                                spacing=0
                            ),
                ...
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Row(
                                [
                                    ft.Icon(ft.Icons.TUNE_ROUNDED, size=16, color=cor_secundaria),
                                    ft.Text("Filtros", size=13, weight=ft.FontWeight.W_600, color=cor_texto_claro),
                                    ft.Container(expand=True),
                                    info_resultados
                                ],
                                alignment=ft.MainAxisAlignment.START,
                            ...
                            ft.Row(
                                [
                                    ...
                                    campo_estado,
                                    ft.ElevatedButton("Limpar", on_click=limpar_filtros, icon=ft.Icons.CLEAR_ALL_ROUNDED, style=ft.ButtonStyle(
                                        bgcolor=cor_fundo, color=cor_texto_claro, shape=ft.RoundedRectangleBorder(radius=8),
                                        elevation=0, padding=ft.padding.symmetric(horizontal=12, vertical=10),
    ...
    # — Footer —
    indicador_pagina_text = ft.Text("Página 1 de 1", size=14, weight=ft.FontWeight.W_600, color=cor_texto_claro)
    campo_pagina = ft.TextField(value="1", width=70, height=40, text_size=14, text_align=ft.TextAlign.CENTER,
                                content_padding=ft.padding.all(8), border_radius=10, border_color=cor_borda,
                                focused_border_color=cor_secundaria, color=cor_texto_claro, bgcolor=cor_fundo,
                                on_submit=on_enter_pagina)
    footer = ft.Container(
        content=ft.Row(
            [
                ft.ElevatedButton("Anterior", on_click=prev_page, style=ft.ButtonStyle(
                    bgcolor=cor_fundo, color=cor_texto_claro, shape=ft.RoundedRectangleBorder(radius=10),
                    elevation=0, padding=ft.padding.symmetric(horizontal=20, vertical=12),
                    overlay_color=ft.Colors.with_opacity(0.1, cor_secundaria)
                ...
                ft.Container(
                    content=ft.Row(
                        [indicador_pagina_text,
                         ft.Container(content=ft.Container(width=1, height=24, bgcolor=cor_borda), margin=ft.margin.symmetric(horizontal=16)),
                         ft.Text("Ir para:", size=13, color=cor_texto_medio),
                         campo_pagina,
                         ft.IconButton(icon=ft.Icons.ARROW_FORWARD_ROUNDED, icon_color=ft.Colors.WHITE, bgcolor=cor_secundaria, on_click=ir_para_pagina, tooltip="Ir para página", icon_size=18)
                        ],
                ...
                ft.Container(expand=True),
                ft.ElevatedButton("Seguinte", on_click=prox_page, style=ft.ButtonStyle(
                    bgcolor=cor_secundaria, color=ft.Colors.WHITE, shape=ft.RoundedRectangleBorder(radius=10),
                    elevation=2, padding=ft.padding.symmetric(horizontal=20, vertical=12),
    ...
    return ft.Container(
        content=ft.Column([header, ft.Container(height=12), lista, ft.Container(height=12), footer], expand=True, spacing=0),
        bgcolor=cor_fundo,
        padding=16,
        expand=True
    )
